...
def pig_actions_d(state):
    ...
    foo = ['double'] if double == 1 else []
    normal = ['roll', 'hold'] if pending else ['roll']
    normal.extend(foo)
    return normal

def strategy_d(state):
    # your code here
    return best_action(state, pig_actions_d, Q_pig_d, U_pig_d)

#========== Solution from the forum ===============
def strategy_d_2(state):
    (p, me, you, pending, double) = state
    if 'double' in pig_actions_d(state) and me + pending >= goal:
        return 'double'
    else:
...
def Q_pig_d(state, action, Pwin):
    ...
    p, me, you, pending, double = state
    if action == 'roll':
        return (1 - Pwin((other[p], you, me+1, 0, double))
                + sum(Pwin((p, me, you, pending+d, double)) for d in (2, 3, 4, 5, 6))) / 6.
    if action == 'decline':
        return 0
    else:
    #if action == 'hold':
...
def U_pig_d(state):
    ...
    if me + pending >= goal:
        ...
        return max(Q_pig_d(state, action, U_pig_d)
...
def hold_20_d(state):
    ...
    return ('accept' if double == 'double' else
            'hold' if (pending >= 50 or me + pending >= goal) else
            'roll')
...
def test():
    ...
    assert strategy_compare(strategy_d_2, hold_20_d) > 60 # must win 60% of the points      
    return 'test passes'
