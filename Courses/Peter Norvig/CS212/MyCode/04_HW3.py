...
def subway(**lines):
    ...
    def adjacent(stop):
        ...
        for linename in lines.keys():
            line = lines[linename].split(' ')
            N = len(line); 
            try :   
                i = line.index(stop); nextstop = []
                if i >= 1 : nextstops[line[i-1]] = linename
                if i < N-1: nextstops[line[i+1]] = linename
            except ValueError:
                pass
        return (stop, nextstops)

    allstop = set().union(*[line.split(' ') for line in lines.values()])
    adjacency = [adjacent(stop) for stop in allstop]
    return dict(adjacency)

## PN's elegant solution:
#==============================
def subway2(**lines):
    successors = collections.defaultdict(dict) # dicts' dict
...
def overlapping_pairs(items):
    # Note how the check of boundary [0, N-1] is avoided
    # by using [i,i+1], [i+1,i+2],... instead of [i-1, i+1]
...
def ride(here, there, system=boston):
    ...
    ## your code here
    def isgoal(stop) : return stop == there
    def subsuccessors(stop):
        return system[stop]
    return shortest_path_search(here, subsuccessors, isgoal)
...
def longest_ride(system):
    ...
    ## your code here
    stops = system.keys()
    paths = [ride(s1, s2, system)
        for s1 in stops for s2 in stops if s1 != s2]
