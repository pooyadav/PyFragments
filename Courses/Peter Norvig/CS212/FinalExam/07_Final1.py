...
def bowling(balls):
    ...
    def checkflag(n):
        a, b = balls[n:n+2]
        if a >= 10 : return 'strike'
        if a + b >= 10 : return 'spare'
        return 'Nothing'
    ...
    for _ in range(10):
        flag = checkflag(ns)
        if flag == 'strike': 
            score += sum(balls[ns:ns+3])
            ns += 1
        elif flag == 'spare':
            score += sum(balls[ns:ns+3])
            ns += 2
        else:
            score += sum(balls[ns:ns+2])
            ns += 2
    #if flag == 'strike' or flag == 'spare': return score
    #return score + balls[ns]
    return score
