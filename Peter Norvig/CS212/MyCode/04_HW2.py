...
def more_pour_problem(capacities, goal, start=None):
    ...
    # I really doubt I have solved this problem previously

    def pgoal(state):
        #return True if goal in state else False
        return goal in state

    # Obviously this version if fucking ugly.
#    def psucc(state):
#        d = dict()
#        for i in range(N):
#            new_state = list(state)[:]
#            new_state[i] = capacities[i]
#            d[tuple(new_state)] = ('fill', i)
#        for i in range(N):
#            new_state = list(state)[:]
#            new_state[i] = 0
#            d[tuple(new_state)] = ('empty', i)
#        for i in range(N):
#            for j in range(N):
#                if i != j:
#                    new_state = list(state)[:]
#                    a, b, C = new_state[i], new_state[j], capacities[j]
#                    if a + b <= C:
#                        new_state[i], new_state[j] = 0, a + b
#                    else:
#                        new_state[i], new_state[j] = a+b-C, C
#                    d[tuple(new_state)] = ('pour', i, j)
#        return d
    ...
    def psucc(state):
        ...
        for i in indices :
            ...
            # So it means ugliness is unavoidable here
            # You don't have too much space for reduction
            for j in indices:
                if i != j:
    ...
    N = len(capacities)
    if start == None : start = (0,) * N
    return shortest_path_search(start, psucc, pgoal)
