...
def boggle_words(board, minlength=3):
    ...
    # your code here
    return reduce(set.union, [explore_from(i, board[i], set(), set([i]), board, minlength) for i in range(len(board))], set())

def explore_from(pos, pre, results, explored, board, minlength):
    if board[pos] == BORDER : return set()
    N = size(board)
    neighborsloc = neighbors(pos, N)
    for i in neighborsloc:
        l = board[i]
        if i not in explored:
            explored.add(i)
            if l != BORDER and (pre+l) in WORDS and len(pre+l) >= minlength:
                results.add(pre+l)
            if l != BORDER and (pre+l) in PREFIXES:
                result = explore_from(i, pre+l, results, explored, board, minlength)
            explored.remove(i) ## !!!!!!
    return results

### ============ PN's solution ==========
### Far far far more better than mine 
def boggle_words2(board, minlength=3):
    result = set()
    N = size(board)
