...
prev_dict = dict(prev_hand = prev_results)

def find_prefixes(hand, pre='', results=None):
    """Find all prefixes (of words) that can be made from letters in hand."""
    if results is None: results = set()
    if hand in prev_dict.keys() and pre == '':
        return prev_dict[hand]
    if pre in PREFIXES or pre in WORDS:
        results.add(pre)
        for L in hand:
            find_prefixes(hand.replace(L, '', 1), pre+L, results)
    prev_dict[hand] = results
    return results

#### ============= PN's solution =============
# I find it a little bit confussing, though
def find_prefixes2(hand, pre='', results = None):
    ## Cache the most recent full hand (not intermediate results)
    global prev_hand, prev_results
    if hand == prev_hand: return prev_results
    if results is None: results = set()
    if pre == '': prev_hand, prev_result = hand, results
    if pre in PREFIXES or pre in WORDS:
        results.add(pre)
        for L in hand:
            find_prefixes(hand.replace(L, '', 1), pre+L, results)
