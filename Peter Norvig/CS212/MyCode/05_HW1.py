...
def Q_pig3(state, action, Pwin):
    "The expected value of choosing action in state."
    if action == 'hold':
        return 1 - Pwin(*hold3(state))
    if action == 'roll':
        return (1 - Pwin(*roll3(state, 1))
                + sum(Pwin(*roll3(state, d)) for d in (2, 3, 4, 5, 6))) / 6.
    raise ValueError
...
def Pwin3(me, you, pending):
   ...
   if me + pending >= goal:
      return 1
       ...
       return max(Q_pig3((me, you, pending), action, Pwin3)
